                             ...
                             uint32_t next_id)
{
  if(node->get_type() == BlockType::BASIC)
  {
    ...
    {
      *created =
          new SelfLoopBlock(next_id, static_cast<const BasicBlock*>(node));
                          ...
                          std::vector<std::unordered_set<uint32_t>>* preds)
{
  if(node->get_out_edges() == 2)
  {
    ...
    while(thenb->get_out_edges() == 2 && marked.find(tnb_id) == marked.end())
    {
            ...
            uint32_t* next_scc, std::vector<uint32_t>* scc)
{
  ...
  *next_index = (*next_index) + 1;
  ...
  for(const AbstractBlock* successor : successors)
  {
    ...
    {
      s_conn(successor, index, lowlink, onstack, stack, next_index, next_scc,
             scc);
                                       ...
                                       uint32_t nodes)
{
  ...
  for(uint32_t i = 0; i < nodes; i++)
  {
    ...
    {
      s_conn(array[i], index, lowlink, onstack, &stack, &next_int, &next_scc,
             &scc);
    }
  }
                                       ...
                                       uint32_t nodes)
{
  ...
  std::unordered_set<uint32_t>* pred = new std::unordered_set<uint32_t>[nodes];
