...
TEST(Comparison, failed_analysis)
{
    ...
    Analysis anal("", std::shared_ptr<Architecture>{new ArchitectureUNK()},
                  out);
...
TEST(Comparison, cloned_partial)
{
    ...
    EXPECT_EQ(original->get_type(), SEQUENCE);
    ...
    EXPECT_EQ((*original)[2]->get_type(), (*cloned)[1]->get_type());

    // check second clone
    rep = clones[1];
    original = orig.get_cfs()->get_node(rep.block_id);
    cloned = check.get_cfs()->get_node(rep.cloned_id);
    EXPECT_EQ(original->get_type(), IF_THEN);
    EXPECT_EQ(original->get_type(), cloned->get_type());
    EXPECT_EQ((*original)[0]->get_type(), BASIC);
    EXPECT_EQ((*original)[0]->get_type(), (*cloned)[1]->get_type());
    EXPECT_EQ((*original)[1]->get_type(), SELF_LOOP);
    EXPECT_EQ((*original)[1]->get_type(), (*cloned)[0]->get_type());
}
...
TEST(Comparison, cloned_multiple_functions)
{
    ...
    // check first clone
    rep = clones[0];
    const AbstractBlock* original = orig1.get_cfs()->get_node(rep.block_id);
    const AbstractBlock* cloned = check.get_cfs()->get_node(rep.cloned_id);
    ...
    cloned = check.get_cfs()->get_node(rep.cloned_id);
    EXPECT_EQ(original->get_type(), WHILE);
    EXPECT_EQ(original->get_type(), cloned->get_type());
    rep = clones[2];
    original = orig0.get_cfs()->get_node(rep.block_id);
    cloned = check.get_cfs()->get_node(rep.cloned_id);
